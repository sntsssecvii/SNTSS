        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold tracking-tight">
                    {filterStatus === 'pending' ? 'Solicitudes Pendientes' :
                        ...
                        {requests.length === 0 ? (
                            ...
                            requests.map((req) => (
                                <TableRow key={req.uid}>
                                    <TableCell>
                                        <div className="font-medium">{req.nombre} {req.apellidoPaterno} {req.apellidoMaterno}</div>
                                    ...
                                    <TableCell>
                                        <div className="flex gap-2">
                                            <Button variant="ghost" size="sm" onClick={() => openDocument(req.documents.identificacion, `INE - ${req.nombre}`)}>
                                                <FileText className="w-4 h-4 mr-1" /> INE
                                            </Button>
                                            <Button variant="ghost" size="sm" onClick={() => openDocument(req.documents.tarjeton, `Tarjetón - ${req.nombre}`)}>
            ...
            <Dialog open={!!selectedRequest} onOpenChange={(open) => !open && setSelectedRequest(null)}>
                <DialogContent className="max-w-2xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>Valida la información y documentos del usuario.</DialogDescription>
                    </DialogHeader>
                        ...
                        <div className="grid gap-6 py-4">
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    ...
                                    {selectedRequest.nombre} {selectedRequest.apellidoPaterno} {selectedRequest.apellidoMaterno}
                                </div>
                                <div>
                            ...
                            <div className="space-y-4">
                                <h4 className="font-medium text-sm text-slate-900">Documentación Adjunta</h4>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="group relative border rounded-xl overflow-hidden bg-slate-50 transition-all hover:ring-2 hover:ring-red-500/20">
                                        <div className="p-2 border-b bg-white flex items-center justify-between">
                                            <span className="text-xs font-bold text-slate-700">Identificación (INE)</span>
                                            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => openDocument(selectedRequest.documents.identificacion, 'Identificación Oficial')}>
                                                <Eye className="h-3 w-3" />
                                            </Button>
                                        </div>
                                        <div className="h-48 relative bg-slate-200 flex items-center justify-center overflow-hidden">
                                            {selectedRequest.documents.identificacion.toLowerCase().includes('.pdf') ? (
                                                <iframe
                                            ...
                                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                <Button variant="secondary" size="sm" onClick={() => openDocument(selectedRequest.documents.identificacion, 'Identificación Oficial')}>
                                                    Ver en Grande
                                                </Button>
                                            </div>
                                        </div>
                                    ...
                                    <div className="group relative border rounded-xl overflow-hidden bg-slate-50 transition-all hover:ring-2 hover:ring-red-500/20">
                                        <div className="p-2 border-b bg-white flex items-center justify-between">
                                            <span className="text-xs font-bold text-slate-700">Tarjetón de Pago</span>
                                            <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => openDocument(selectedRequest.documents.tarjeton, 'Tarjetón de Pago')}>
                                                <Eye className="h-3 w-3" />
                                            </Button>
                                        </div>
                                        <div className="h-48 relative bg-slate-200 flex items-center justify-center overflow-hidden">
                                            {selectedRequest.documents.tarjeton.toLowerCase().includes('.pdf') ? (
                                                <iframe
                                            ...
                                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                <Button variant="secondary" size="sm" onClick={() => openDocument(selectedRequest.documents.tarjeton, 'Tarjetón de Pago')}>
                                                    Ver en Grande
                                                </Button>
                                            </div>
                                        </div>
                            ...
                            {rejectReason && (
                                <p className="text-sm text-red-500">Razon de rechazo requerida para rechazar.</p>
                            )}
                        ...
                        {filterStatus === 'pending' && (
                            <>
                                <Button variant="destructive" onClick={handleReject} disabled={isProcessing || !rejectReason}>
                                    {isProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <><X className="w-4 h-4 mr-2" /> Rechazar</>}
                                </Button>
                                <Button className="bg-green-600 hover:bg-green-700" onClick={handleApprove} disabled={isProcessing}>
                                    {isProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4 mr-2" /> Aprobar</>}
                                </Button>
                            </>
                        )}
                        {filterStatus === 'active' && (
                            <Button variant="destructive" onClick={handleReject} disabled={isProcessing || !rejectReason}>
                                {isProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <><X className="w-4 h-4 mr-2" /> Desactivar / Rechazar</>}
                            </Button>
                        )}
                        {filterStatus === 'rejected' && (
                            <Button className="bg-green-600 hover:bg-green-700" onClick={handleApprove} disabled={isProcessing}>
                                {isProcessing ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4 mr-2" /> Reactivar / Aprobar</>}
                            </Button>
                        )}
                    </DialogFooter>
                </DialogContent>
                    ...
                    <DialogHeader className="p-4 bg-white/10 text-white flex-row items-center justify-between space-y-0">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <DialogDescription className="text-slate-300 text-xs">Visualización segura de documentos SNTSS</DialogDescription>
                            </div>
                        </div>
                        <Button
                            variant="ghost"
