...
import type { Propuesta, FiltrosPropuesta, CategoriaPropuesta } from '@/types/propuestas'
...
export function PropuestasTable({ onNuevaPropuesta }: PropuestasTableProps) {
  ...
  const [showEditarPropuestaModal, setShowEditarPropuestaModal] = useState(false)
    ...
    <>
      ...
      <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
          ...
          <Button onClick={() => setShowNuevaPropuestaModal(true)} className="h-10 sm:h-11 w-full sm:w-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          <div>
            <label className="text-xs sm:text-sm font-medium mb-1.5 sm:mb-2 block">Categoría</label>
            <Select
          ...
          <div>
            <label className="text-xs sm:text-sm font-medium mb-1.5 sm:mb-2 block">Fecha desde</label>
            <Input
              type="date"
              value={
                filtros.fechaDesde
          ...
          <div>
            <label className="text-xs sm:text-sm font-medium mb-1.5 sm:mb-2 block">Fecha hasta</label>
            <Input
              type="date"
              value={
                filtros.fechaHasta
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-xs sm:text-sm">Trabajador</TableHead>
                  <TableHead className="text-xs sm:text-sm">Matrícula</TableHead>
                  <TableHead className="text-xs sm:text-sm">Aspirante</TableHead>
              ...
              <TableBody>
                {propuestas.map((propuesta) => (
                  <TableRow key={propuesta.id}>
                    <TableCell className="text-xs sm:text-sm">{formatearFecha(propuesta.fechaCreacion)}</TableCell>
                    <TableCell className="font-medium text-xs sm:text-sm">
                      {propuesta.trabajadorActivo.nombre}
                    </TableCell>
                    <TableCell className="text-xs sm:text-sm">{propuesta.trabajadorActivo.matricula}</TableCell>
                    <TableCell className="text-xs sm:text-sm">{propuesta.aspirante.nombre}</TableCell>
                    <TableCell>
                ...
                <div className="space-y-3">
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-medium">{formatearFecha(propuesta.fechaCreacion)}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <div>
                      ...
                      <p className="text-sm font-medium truncate">{propuesta.trabajadorActivo.nombre}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Matrícula</p>
                      <p className="text-sm">{propuesta.trabajadorActivo.matricula}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Aspirante</p>
                      <p className="text-sm truncate">{propuesta.aspirante.nombre}</p>
                    </div>
                  </div>
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              ¿Estás seguro de que deseas eliminar esta propuesta? Esta acción no se puede
              deshacer.
            </DialogDescription>
          </DialogHeader>
          {propuestaAEliminar && (
            <div className="py-4 space-y-2 text-sm">
              <p>
                <strong>Trabajador:</strong> {propuestaAEliminar.trabajadorActivo.nombre}
              </p>
              <p>
                <strong>Matrícula:</strong> {propuestaAEliminar.trabajadorActivo.matricula}
              </p>
              <p>
                <strong>Aspirante:</strong> {propuestaAEliminar.aspirante.nombre}
              </p>
            </div>
          )}
          <DialogFooter>
      ...
      >
        <DialogContent className="max-w-7xl w-[95vw] sm:w-[90vw] max-h-[95vh] overflow-hidden flex flex-col p-0 sm:p-6">
          ...
          <DialogHeader className="flex-shrink-0 px-4 sm:px-6 pt-4 sm:pt-6">
            <DialogTitle className="text-xl sm:text-2xl">Nueva Propuesta</DialogTitle>
      ...
      >
        <DialogContent className="max-w-7xl w-[95vw] sm:w-[90vw] max-h-[95vh] overflow-hidden flex flex-col p-0 sm:p-6">
          ...
          <DialogHeader className="flex-shrink-0 px-4 sm:px-6 pt-4 sm:pt-6">
            <DialogTitle className="text-xl sm:text-2xl">Editar Propuesta</DialogTitle>
            <DialogDescription className="text-sm">
