...
export default function RegistroPage() {
  return (
    <div className="min-h-screen w-full relative overflow-hidden bg-slate-50 flex flex-col selection:bg-red-100 selection:text-red-900">
      ...
      <div className="container mx-auto px-0 sm:px-4 py-4 sm:py-10 relative z-10 flex-1 flex flex-col max-w-7xl">
        {/* Superior Header */}
        <motion.header
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
          className="flex justify-between items-center mb-4 sm:mb-16 px-4 sm:px-0"
        >
          ...
          <Link
            href="/login"
            className="text-sm font-bold text-slate-600 hover:text-red-700 transition-colors"
          >
            ¿Ya tienes cuenta?{" "}
            <span className="text-red-700 font-black ml-1">Ingresar</span>
          </Link>
        </motion.header>

        {/* Main Body */}
        <div className="flex-1 flex flex-col items-center justify-center sm:pb-20">
          <div className="w-full max-w-3xl sm:space-y-12">
            <div className="hidden sm:block text-center space-y-6">
              <motion.div
            ...
            >
              <div className="hidden sm:block absolute -inset-2 bg-gradient-to-br from-red-500/10 via-transparent to-orange-500/10 rounded-[3rem] blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
              <div className="relative bg-white/95 sm:border sm:border-white/60 sm:rounded-[2.5rem] sm:shadow-[0_32px_64px_-12px_rgba(0,0,0,0.08)] sm:p-px">
                <div className="overflow-hidden sm:rounded-[2.5rem]">
                  <RegistroForm />
                </div>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              className="hidden sm:grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto pt-6"
            >
              {["Seguridad", "Transparencia", "Rapidez", "Oficial"].map(
                (tag) => (
                  <div
                    key={tag}
                    className="flex flex-col items-center gap-2 p-4 rounded-2xl bg-slate-100/50 border border-slate-200/40"
                  >
