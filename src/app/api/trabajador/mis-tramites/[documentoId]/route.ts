...
export async function GET(
  ...
  try {
    ...
    const recordId =
      request.nextUrl.searchParams.get("recordId")?.trim() || null;
    ...
    if (documentoSnap.get("syncId") !== syncActiva.id) {
      return NextResponse.json(
        { error: "El trámite no pertenece al corte oficial vigente." },
        { status: 404 },
      );
    }
    ...
    const resultado = posiciones.find(
      (item) =>
    ...
    if (!resultado) {
      ...
      return NextResponse.json(
        { error: "El trámite solicitado no pertenece al usuario autenticado." },
        { status: 403 },
      );
    }
