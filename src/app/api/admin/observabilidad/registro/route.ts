...
export async function GET(request: NextRequest) {
  try {
    enforceRateLimit(request, { bucket: 'api:admin:observabilidad:registro', limit: 30, windowMs: 60_000 })
    ...
    const [
      ...
      adminDb.collection('registration_audit_logs').where('createdAt', '>=', oneHourAgo).get(),
      adminDb.collection('registration_audit_logs').orderBy('createdAt', 'desc').limit(12).get(),
      adminDb.collection('admin_audit_logs').orderBy('createdAt', 'desc').limit(20).get(),
    ])
    ...
    const adminLastHour = adminRecentSnap.docs.reduce(
      (acc, doc) => {
        ...
        if (data.action === 'USER_VALIDATION_UPDATED' && data.metadata?.nextStatus === 'active') {
          acc.approved += 1
        }

        if (data.action === 'USER_VALIDATION_UPDATED' && data.metadata?.nextStatus === 'rejected') {
          acc.rejected += 1
        }
    ...
    const registrationEvents = registrationRecentSnap.docs.map((doc) => {
      ...
      return {
        id: doc.id,
        source: 'registration',
        status: data.status === 'ERROR' ? 'error' : warning ? 'warning' : 'success',
    ...
    const adminEvents = adminRecentSnap.docs
      .map((doc) => {
        ...
        const status = data.status === 'ERROR' ? 'error' : nextStatus === 'rejected' ? 'warning' : 'success'
    ...
    return NextResponse.json(
      { error: 'No se pudo obtener la observabilidad de registro.', details: error?.message || 'UNKNOWN_ERROR' },
      { status: 500 }
    )
  }
}
