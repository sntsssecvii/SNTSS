...
export async function POST(request: NextRequest) {
  try {
    enforceRateLimit(request, { bucket: 'api:admin:lab:chat-contrato', limit: 20, windowMs: 60_000 })
    ...
    if (error?.message === 'QUERY_REQUIRED') {
      return NextResponse.json({ error: 'Escribe una pregunta para consultar el contrato.' }, { status: 400 })
    }

    return NextResponse.json(
      {
        error: error?.message || 'No se pudo resolver la consulta del contrato.',
      },
      { status: 500 }
    )
  }
