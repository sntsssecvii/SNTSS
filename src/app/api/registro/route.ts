...
async function uploadRegistrationFile(userUid: string, file: File, type: 'identificacion' | 'tarjeton') {
...
export async function POST(request: NextRequest) {
  ...
  try {
    enforceRateLimit(request, { bucket: 'api:registro:create:ip', limit: 40, windowMs: 5 * 60_000 })
    ...
    const [existingEmailProfile, existingMatriculaProfile] = await Promise.all([
      adminDb.collection('users').where('emailLowercase', '==', searchFields.emailLowercase).limit(1).get(),
      adminDb.collection('users').where('matriculaNormalized', '==', searchFields.matriculaNormalized).limit(1).get(),
    ])
    ...
    if (!existingMatriculaProfile.empty) {
      return NextResponse.json({ error: 'Ya existe una solicitud registrada para esta matrícula.' }, { status: 409 })
    }
    ...
    try {
      ...
      emailWarning = 'La solicitud se registró, pero no se pudo enviar el correo de confirmación.'
    }
    ...
    await writeRegistrationAuditLog({
      ...
      console.error('Error escribiendo auditoría de registro exitoso:', auditError)
    })
    ...
    if (createdUid) {
      try {
        ...
        console.error('Error limpiando usuario huérfano en Auth:', cleanupError)
      }
    }
    ...
    await writeRegistrationAuditLog({
      ...
      console.error('Error escribiendo auditoría de registro fallido:', auditError)
    })
    ...
    if (typeof error?.message === 'string') {
      if (error.message.endsWith('_REQUIRED')) {
        return NextResponse.json({ error: 'Faltan documentos requeridos para completar el registro.' }, { status: 400 })
      }

      if (error.message.endsWith('_INVALID_TYPE')) {
        return NextResponse.json({ error: 'Solo se aceptan archivos JPG, PNG, WEBP o PDF.' }, { status: 400 })
      }

      if (error.message.endsWith('_INVALID_SIZE')) {
        return NextResponse.json({ error: 'Cada archivo debe pesar máximo 5 MB.' }, { status: 400 })
      }
    }

    return NextResponse.json(
      { error: 'No se pudo completar el registro.', details: error?.message || 'UNKNOWN_ERROR' },
      { status: 500 }
    )
  }
}
