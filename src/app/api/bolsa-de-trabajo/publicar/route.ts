...
export async function POST(request: NextRequest) {
  ...
  try {
    enforceRateLimit(request, { bucket: 'api:bolsa:publicar', limit: 6, windowMs: 60_000 })
    ...
    const { totalDocumentos, totalMaterializados } = await materializeSyncPositions(syncId, periodo)
    ...
    if (adminUser) {
      try {
        ...
        console.error('Error registrando auditoría de publicación:', auditError)
      }
    }
    ...
    if (error?.message === 'SYNC_WITHOUT_DOCUMENTS') {
      return NextResponse.json({ error: 'La sincronización no tiene documentos para publicar.' }, { status: 400 })
    }
    ...
    if (typeof error?.message === 'string' && error.message.startsWith('SYNC_HAS_INCOMPLETE_DOCUMENTS:')) {
      ...
      return NextResponse.json(
        {
          error: 'Todos los documentos deben estar completados antes de publicar.',
          detalles,
        },
        { status: 400 }
      )
    ...
    return NextResponse.json(
      { error: 'No se pudo publicar la sincronización.', details: error?.message || 'UNKNOWN_ERROR' },
      { status: 500 }
    )
  }
}
