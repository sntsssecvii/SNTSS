  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      if (file) {
        ...
        const headers =
          data[0]?.map((h: string) => h?.toString().toLowerCase().trim()) || [];
        ...
        for (let i = 1; i < data.length; i++) {
          ...
          const validado =
            validadoStr.toLowerCase() === "sí" ||
            validadoStr.toLowerCase() === "si" ||
            validadoStr.toLowerCase() === "yes" ||
            validadoStr === "1";
        ...
        const nombreArchivo =
          (formData.get("nombreArchivo") as string) || file.name;
    ...
    if (error?.message === "ADMIN_REQUIRED") {
      return NextResponse.json(
        { error: "No tienes permisos para importar información." },
        { status: 403 },
      );
    }
  ...
  return NextResponse.json({
    ...
    metodos: {
      POST: {
        ...
        json: {
          tipoDocumento:
            "Tipo de documento (NUEVO_INGRESO, AMPLIACIONES_JORNADA, etc.)",
