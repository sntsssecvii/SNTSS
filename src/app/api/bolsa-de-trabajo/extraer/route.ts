...
async function extraerDatosCrudos(buffer: Buffer, nombreArchivo: string): Promise<{
  filas: FilaCruda[]
  tipoDocumento: string
}> {
  ...
  const extractionResult = await parsePDF(buffer, 'NUEVO_INGRESO', nombreArchivo)

  // Re-extraemos el texto si es necesario o usamos lo que ya tenemos
  // Pero aquí procesamos linea por linea
  const texto = extractionResult.registros.length > 0 ?
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { filas, tipoDocumento: tipoDetectado } = await extraerDatosCrudos(buffer, file.name)
    ...
    const response: ExtraerResponse = {
      ...
      registrosEncontrados: filas.filter(f => f.tipoLinea === 'registro').length,
      filas
    }
