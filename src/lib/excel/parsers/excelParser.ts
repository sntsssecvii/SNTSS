    ...
    try {
        ...
        switch (tipo) {
            ...
            case 'CAMBIOS_RESIDENCIA_DESTINO':
                resultado = parseCambiosResidenciaExcel(workbook, 'CAMBIOS_RESIDENCIA_DESTINO');
                break;
            case 'CAMBIOS_RESIDENCIA_ORIGEN':
                resultado = parseCambiosResidenciaExcel(workbook, 'CAMBIOS_RESIDENCIA_ORIGEN');
                break;
            ...
            if (isNumericNoProg && row.length >= 3 && col1 && col2) {
                try {
                    ...
                    if (validationErrors.length > 0) {
                        errores.push(`Hoja ${sheetName}, Fila ${index + 1}: ${validationErrors.join('; ')}`);
                    }

                    registros.push(registro);
                } catch (e: any) {
                    errores.push(`Error en Hoja ${sheetName}, Fila ${index + 1}: ${e.message}`);
                }
                return;
            }
        ...
        rows.forEach((row, index) => {
            ...
            if (isNumericNoProg && col9 && (col10 || col1)) {
                try {
                    ...
                    else if (rawDias) diasLaborados = String(rawDias).trim().replace(/,/g, '');
 ...
 */
function parseAmpliacionesJornadaExcel(workbook: XLSX.WorkBook): ExcelParseResult {
    const registros: BolsaDeTrabajoRegistro[] = [];
    const errores: string[] = [];
    ...
    workbook.SheetNames.forEach((sheetName) => {
        ...
        rows.forEach((row, index) => {
            ...
            if (!col0) return;
            const col0Norm = col0.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toUpperCase();
            const col1 = String(row[1] || '').trim();
            const col2 = String(row[2] || '').trim();

            // Zona
            if ((/^\d{1,2}-\S/.test(col0) || col0Norm.startsWith('ZONA')) && !col1 && !col2) {
                zonaActual = col0.replace(/^zona:?\s*/i, '').trim(); return;
            }
            ...
            if (isNumericNoProg && col7 && col8) {
                try {
                    ...
                    const registro: BolsaDeTrabajoRegistro = {
                        ...
                        adscripcionAnterior: [String(row[10] || '').trim(), String(row[11] || '').trim()].filter(Boolean).join(' - ') || undefined,
    ...
    return { registros, metadata: { totalRegistros: registros.length, extraidoCon: 'EXCEL', zona: zonaActual }, errores };
}
...
function parseCambiosRamaExcel(workbook: XLSX.WorkBook): ExcelParseResult {
    ...
    workbook.SheetNames.forEach((sheetName) => {
        ...
        rows.forEach((row, index) => {
            ...
            const isNumericNoProg = /^\d+$/.test(col0);
            const col6 = String(row[6] || '').trim(); // Matrícula
            const col7 = String(row[7] || '').trim(); // Nombre
            if (isNumericNoProg && col6 && col7) {
                try {
                    const fechaRaw = row[3];
                    const fecha = typeof fechaRaw === 'number' ? excelDateToString(fechaRaw) : String(fechaRaw || '').trim();

                    const adscAnt = [String(row[9] || '').trim(), String(row[11] || '').trim()].filter(Boolean).join(' - ');
                    const registro: BolsaDeTrabajoRegistro = {
                        ...
                        fecha,
                        estatus: String(row[4] || '').trim() || undefined,
                        numeroPlaza: String(row[5] || '').trim() || undefined,
                        matricula: col6,
                        nombre: col7.replace(/\//g, ' '),
                        sexo: String(row[8] || '').trim() || undefined,
                        adscripcionAnterior: adscAnt || undefined,
                        diasLaborados: String(row[14] || '').trim() || undefined,
 ...
 */
function parseCambiosResidenciaExcel(workbook: XLSX.WorkBook, tipo: 'CAMBIOS_RESIDENCIA_DESTINO' | 'CAMBIOS_RESIDENCIA_ORIGEN'): ExcelParseResult {
    const registros: BolsaDeTrabajoRegistro[] = [];
    const errores: string[] = [];
        ...
        rows.forEach((row, index) => {
            ...
            if (isNumericNoProg && col7 && col8) {
                try {
                    ...
                    const adscAnt = [String(row[10] || '').trim(), String(row[12] || '').trim()].filter(Boolean).join(' - ');
                    const registro: BolsaDeTrabajoRegistro = {
                        id: parseUtils.generarIdRegistro(tipo, registros.length),
                        tipoDocumento: tipo,
                        numeroProg: col0,
                        delegacionDestino: tipo === 'CAMBIOS_RESIDENCIA_DESTINO' ? col1 || undefined : undefined,
                        delegacionOrigen: tipo === 'CAMBIOS_RESIDENCIA_ORIGEN' ? col1 || undefined : undefined,
                        turnoNuevo: String(row[3] || '').trim() || undefined,
                        fecha,
                        estatus: String(row[5] || '').trim() || undefined,
                        diasLaborados: String(row[6] || '').trim() || undefined,
 ...
 */
function parseGenericoExcel(workbook: XLSX.WorkBook, tipo: TipoBolsaDeTrabajo): ExcelParseResult {
    const registros: BolsaDeTrabajoRegistro[] = [];
    const errores: string[] = [];
    let zonaActual = '';
    let categoriaActual = '';
    ...
    workbook.SheetNames.forEach((sheetName) => {
        ...
        for (let i = 0; i < Math.min(rows.length, 20); i++) {
            ...
            if (rowStr.includes('NOMBRE') && (rowStr.includes('MATRICULA') || rowStr.includes('MATRÍCULA'))) {
                headers = rowStr;
                headerRowIndex = i;
                break;
            }
        ...
        for (let i = headerRowIndex + 1; i < rows.length; i++) {
            ...
            const isZonePattern = /^\d+-/.test(firstCol) || firstCol.toUpperCase().includes('ZONA');
            ...
            if (row.length >= 2) {
                ...
                const adscripcionIdx = headers.findIndex(h => h.includes('ADSCRIPCION') || h.includes('ADSCRIPCIÓN'));
                const fechaIdx = headers.findIndex(h => h.includes('FECHA'));

                const nombre = nombreIdx !== -1 ? String(row[nombreIdx] || '').trim() : '';
                const matricula = matriculaIdx !== -1 ? String(row[matriculaIdx] || '').trim() : '';
                ...
                if (nombre && matricula && matricula.length >= 4) {
                    ...
                    const registro: BolsaDeTrabajoRegistro = {
                        ...
                        matricula,
                        fecha: fechaStr,
                        adscripcion: adscripcionIdx !== -1 ? String(row[adscripcionIdx] || '').trim() : '',
                        zona: zonaActual,
                    ...
                    headers.forEach((h, idx) => {
                        if (h.includes('TURNO')) registro.turnoActual = String(row[idx] || '').trim();
                        if (h.includes('JORNADA')) registro.jornadaActual = String(row[idx] || '').trim();
                        if (h.includes('PLAZA')) registro.numeroPlaza = String(row[idx] || '').trim();
                    });
