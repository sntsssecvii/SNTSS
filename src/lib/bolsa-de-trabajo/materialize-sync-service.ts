...
import { upsertBolsaPosicionesMaterializadas } from '@/lib/firebase/bolsa-posiciones-materializadas'
import type { BolsaDeTrabajoRegistro, PeriodoBolsa, TipoBolsaDeTrabajo } from '@/types/bolsa-de-trabajo'
...
export async function materializeSyncPositions(syncId: string, periodo: PeriodoBolsa): Promise<MaterializeSyncResult> {
  ...
  const documentosNoCompletados = documentos.filter((doc) => doc.estado !== 'COMPLETADO')
  if (documentosNoCompletados.length > 0) {
    throw new Error(`SYNC_HAS_INCOMPLETE_DOCUMENTS:${JSON.stringify(documentosNoCompletados)}`)
  }
