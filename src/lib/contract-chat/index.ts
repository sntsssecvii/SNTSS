...
function tokenizeQuery(query: string) {
  return Array.from(new Set(tokenizeNormalizedText(normalizeText(query))));
}
...
function buildAnswerText(query: string, sources: ContractSearchResult[]) {
  ...
  const outro =
    "Respuesta generada en modo sandbox extractivo. Conviene validar el contexto completo en las páginas citadas antes de usarlo como criterio operativo.";
...
function scoreChunk(
  ...
  const isIndexChunk = INDEX_MARKERS.some((marker) =>
    chunk.normalizedText.includes(marker),
  );
  ...
  const chunkTokenTotal =
    Object.values(chunk.tokenCounts).reduce((sum, count) => sum + count, 0) ||
    1;
  ...
  for (const token of tokens) {
    ...
    const inverseDocumentFrequency = Math.log(
      (index.chunkCount + 1) / documentFrequency,
    );
...
async function loadPersistedContractIndex() {
  ...
  // En desarrollo, si el PDF está disponible, validar frescura por mtime
  if (fs.existsSync(CONTRACT_PATH)) {
    const contractStats = await fs.promises.stat(CONTRACT_PATH);
...
export async function answerContractQuestion(
  ...
  const rankedSources = index.chunks
    ...
    .sort(
      (a, b) => b.score - a.score || a.chunk.pageNumber - b.chunk.pageNumber,
    )
    .slice(0, 6);
  ...
  try {
    ...
    console.error(
      "Groq no disponible para sandbox de contrato, usando fallback extractivo:",
      error,
    );
  }
