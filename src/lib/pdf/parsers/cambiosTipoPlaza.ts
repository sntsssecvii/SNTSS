...
export function parseCambiosTipoPlaza(texto: string): ParseResult {
  ...
  for (let i = 0; i < lineas.length; i++) {
    ...
    if (match) {
      const [
        , numeroProg, cambio, _adscripActClave, tipoPlazaAnterior,
        fecha, registro, _dias, matricula, nombre, sexo,
        _adscripNuevaClave, _adscripNuevaNombre, _plaza, _jornada, tipoPlazaNuevo,
      ] = match
    ...
    const turnoAnteriorIdx = partes.findIndex((p, idx) => idx < fechaIdx && /^(Mat|Ves|Noc|JAcum|Acum)$/.test(p))
    ...
    const registroObj: BolsaDeTrabajoRegistro = {
      ...
      tipoDocumento: 'CAMBIOS_TIPO_PLAZA',
      numeroProg: partes[0],
      tipoPlazaAnterior: turnoAnteriorIdx !== -1 ? partes[turnoAnteriorIdx] : '',
