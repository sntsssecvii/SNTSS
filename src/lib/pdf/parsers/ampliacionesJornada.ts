...
const REGEX_ESTRICTO =
  /^(\d+)\s+(\d+)\s+([A-Z0-9]+)\s+([A-Za-z]+)\s+(\d{2}\/\d{2}\/\d{4})\s+([A-Z])\s+([\d,]+)\s+(\d{7,10})\s+(.+?)\s+([MF])(?:\s+([A-Z0-9]+)\s+(.+?)\s+(\d+)\s+([\d.]+)\s+([A-Za-z]+))?$/;

export function parseAmpliacionesJornada(
  texto: string,
): BolsaDeTrabajoParseResult {
  ...
  for (let i = 0; i < lineas.length; i++) {
    ...
    if (match) {
      ...
      const registroObj: BolsaDeTrabajoRegistro = {
        ...
        turnoNueva: turnoNueva || undefined,
        zona: zonaActual,
    ...
    if (partes.length >= 10 && /^\d+$/.test(partes[0])) {
      ...
      if (matIdx !== -1) {
        ...
        if (validationErrors.length > 0) {
          errores.push(
            `Fila ${i + 1} (flexible): ${validationErrors.join("; ")}`,
          );
        }
