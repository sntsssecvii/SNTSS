...
export const getPropuestasPorEstado = async (estado: EstadoPropuesta): Promise<number> => {
  try {
...
export const getPropuestasPorCategoria = async (categoria: CategoriaPropuesta): Promise<number> => {
  try {
    const q = query(
    ...
    console.error(`Error obteniendo propuestas por categoría ${categoria}:`, error)
    return 0
  }
}

// Obtener distribución de propuestas por estado
export const getDistribucionPorEstado = async (): Promise<Record<EstadoPropuesta, number>> => {
  try {
...
export const getPropuestasPorMes = async (): Promise<Array<{ mes: string; cantidad: number }>> => {
  try {
...
export const getPropuestasPendientes = async (): Promise<number> => {
  try {
    const enRevision = await getPropuestasPorEstado(EstadoPropuesta.EN_REVISION)
...
export const getActividadDelMes = async (): Promise<number> => {
  try {
    const ahora = new Date()
    const inicioMes = Timestamp.fromDate(new Date(ahora.getFullYear(), ahora.getMonth(), 1))
    const finMes = Timestamp.fromDate(
...
export const getTiempoPromedioProcesamiento = async (): Promise<number> => {
  try {
    const q = query(
      collection(db, COLECCION),
      where('estado', 'in', [EstadoPropuesta.APROBADA, EstadoPropuesta.COMPLETADA])
    )
    ...
    snapshot.forEach((doc) => {
      ...
      if (data.fechaCreacion && data.fechaActualizacion) {
        ...
        const dias = Math.ceil(
          (fechaActualizacion.getTime() - fechaCreacion.getTime()) / (1000 * 60 * 60 * 24)
        )
        totalDias += dias
        count++
      }
...
export const getTopCategorias = async (limite: number = 5): Promise<Array<{ categoria: string; cantidad: number }>> => {
  try {
