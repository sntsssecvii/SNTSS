...
import type { CategoriaPropuesta } from "@/types/propuestas";
...
> => {
  try {
    const estados: EstadoPropuesta[] = ["PENDIENTE", "APROBADA", "RECHAZADA"];
...
export const getTasaAprobacion = async (): Promise<number> => {
  try {
    ...
    const q = query(
      collection(db, COLECCION),
      where("actualizadoEn", ">=", hace30Dias),
    );
...
export const getTiempoPromedioProcesamiento = async (): Promise<number> => {
  try {
    ...
    snapshot.forEach((doc) => {
      const data = doc.data();
      if (data.creadoEn && data.actualizadoEn) {
        const fechaCreacion = convertirTimestamp(data.creadoEn);
        const fechaActualizacion = convertirTimestamp(data.actualizadoEn);
