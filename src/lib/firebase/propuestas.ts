...
const convertirPropuesta = (doc: any): Propuesta => {
  ...
  return {
    ...
    fechaVencimiento: data.fechaVencimiento ? convertirTimestamp(data.fechaVencimiento) : undefined,
...
export const getPropuestaById = async (id: string): Promise<Propuesta | null> => {
  try {
...
const detectarCamposCoincidentes = (
  ...
  if (t1.matricula && t2.matricula && sonSimilares(t1.matricula, t2.matricula)) {
  ...
  try {
    ...
    if (filtros.localidad) {
      ...
      resultados = resultados.filter(
        (p) =>
          normalizar(p.trabajadorActivo.localidad).includes(localidadBusqueda) ||
