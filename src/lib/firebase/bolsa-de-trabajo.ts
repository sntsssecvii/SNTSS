...
const convertirDocumento = (
  doc: any,
  incluirRegistros: boolean = false,
): BolsaDeTrabajoDocumento => {
  const data = doc.data();
  return {
    id: doc.id,
...
export const createBolsaDeTrabajoDocumento = async (
  ...
  try {
    ...
    const nuevoDocumento = {
      ...
      fechaActualizacion:
        documento.fechaActualizacion instanceof Date
...
export const guardarRegistrosEnSubcoleccion = async (
  ...
  try {
    ...
    for (let i = 0; i < registrosLimpios.length; i += BATCH_SIZE) {
      ...
      await batch.commit();
      console.log(
        `Guardados ${Math.min(i + BATCH_SIZE, registrosLimpios.length)}/${registrosLimpios.length} registros`,
      );
    }
  } catch (error) {
    ...
    if (filtros?.fechaDesde) {
      constraints.push(
        where(
          "fechaActualizacion",
          ">=",
          Timestamp.fromDate(filtros.fechaDesde),
        ),
      );
    }
    ...
    if (filtros?.fechaHasta) {
      constraints.push(
        where(
          "fechaActualizacion",
          "<=",
          Timestamp.fromDate(filtros.fechaHasta),
        ),
      );
    }
...
export const getBolsaDeTrabajoDocumentosBySyncIds = async (
  ...
  try {
    ...
    for (let i = 0; i < syncIds.length; i += CHUNK_SIZE) {
      ...
      const querySnapshot = await getDocs(q);
      const chunkDocs = querySnapshot.docs.map((doc) =>
        convertirDocumento(doc),
      );
    ...
    console.error(
      "Error obteniendo documentos por múltiples sincronizaciones:",
      error,
    );
    throw error;
  }
...
export const getBolsaDeTrabajoDocumentoBySyncAndTipo = async (
  ...
  try {
    ...
    console.error(
      "Error obteniendo documento por sincronización y tipo:",
      error,
    );
    throw error;
  }
...
export const deleteBolsaDeTrabajoDocumento = async (
  id: string,
): Promise<void> => {
  try {
    const docRef = doc(db, COLECCION, id);
    ...
    if (!querySnapshot.empty) {
      ...
      console.log(
        `Eliminados ${docs.length} registros de la subcolección para el documento ${id}`,
      );
    }
...
export const getEstadisticasBolsaDeTrabajo =
  async (): Promise<EstadisticasBolsaDeTrabajo> => {
    try {
      ...
      documentos.forEach((doc) => {
        ...
        registrosPendientes +=
          (doc.totalRegistros || 0) - (doc.registrosValidados || 0);
