...
export const getPendingUsers = async (): Promise<(UserData & { id: string })[]> => {
  try {
...
export const getUsersByStatus = async (status: 'active' | 'pending' | 'rejected'): Promise<(UserData & { id: string })[]> => {
  try {
    const q = query(
      collection(db, 'users'),
      where('status', '==', status),
    ...
    const qBasic = query(collection(db, 'users'), where('status', '==', status))
...
export const updateUserStatus = async (userId: string, status: 'active' | 'rejected'): Promise<boolean> => {
  try {
