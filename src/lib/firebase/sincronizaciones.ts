...
const scoreSincronizacion = (sync: Sincronizacion): number => {
  const estadoScore =
    sync.estado === "COMPLETADO" ? 2 : sync.estado === "PROCESANDO" ? 1 : 0;
  const fecha = convertirTimestamp(
    sync.fechaFinalizacion || sync.fechaInicio,
  ).getTime();

  return (
    (sync.esFuenteVerdad ? 1_000_000_000_000 : 0) +
    estadoScore * 1_000_000_000 +
    fecha
  );
};

const seleccionarSincronizacionCanonica = (
  sincronizaciones: Sincronizacion[],
): Sincronizacion | null => {
  if (sincronizaciones.length === 0) return null;

  return [...sincronizaciones].sort(
    (a, b) => scoreSincronizacion(b) - scoreSincronizacion(a),
  )[0];
};
...
export const createSincronizacion = async (
  sync: Omit<
    Sincronizacion,
    "id" | "fechaInicio" | "estado" | "esFuenteVerdad"
  >,
): Promise<string> => {
  try {
    const nuevaSync = {
...
export const getSincronizacionById = async (
  id: string,
...
export const getSincronizacionPorPeriodo = async (
  ...
  try {
    ...
    return seleccionarSincronizacionCanonica(
      querySnapshot.docs.map(convertirSincronizacion),
    );
  } catch (error) {
...
export const getSincronizaciones = async (
  limite: number = 20,
...
export const updateSincronizacion = async (
  ...
  try {
    ...
    if (actualizaciones.fechaFinalizacion instanceof Date) {
      data.fechaFinalizacion = Timestamp.fromDate(
        actualizaciones.fechaFinalizacion,
      );
    }
