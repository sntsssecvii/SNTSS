...
// Credenciales admin en memoria (no sessionStorage) para re-auth al crear usuarios
let _adminCredentials: { email: string; password: string } | null = null;

export const setCreatingUserState = (state: boolean) => {
...
export const initializeAuthListener = () => {
  ...
  currentAuthUnsubscribe = onAuthStateChanged(auth, (_user) => {});
};
...
export const createUserAndMaintainSession = async (
  adminEmail: string,
...
export const createNewUserWithoutSignIn = async (
  email: string,
  password: string,
) => {
  const adminEmail = auth.currentUser?.email;
  const adminPassword = _adminCredentials?.password;
  if (!adminEmail || !adminPassword)
    throw new Error("Credenciales de admin no encontradas");
  return createUserAndMaintainSession(
    adminEmail,
    adminPassword,
    email,
    password,
